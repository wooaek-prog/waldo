...
도면 기반 수광점(위 절)으로 **수광점 하나하나의 충족여부**를 내고, 화랑 재건축
전후를 비교합니다. 두 시나리오 모두 **대교는 신축안이 지어진 상태**로 고정해,
차이가 **화랑 재건축만의 순수 영향**이 되게 했습니다.

```bash
python3 school_receptor_compliance.py --buildings <AL_D010.gpkg>
# → outputs/school_compliance/step1~3 CSV, docs/school_receptor_compliance.md
```

| 학교 | 수광점 | S1 충족률(화랑 기존) | S2 충족률(화랑 신축) | 변화 | 신규 불충족 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 여의도고등학교 | 246 | 87.4% | 87.4% | ±0.0%p | 0 |
| 여의도중학교 | 289 | 81.0% | **70.9%** | **−10.0%p** | 31 |
| 여의도여자고등학교 | 194 | 66.5% | 68.6% | +2.1%p | 13 |
| 여의도초등학교 | 134 | 91.8% | 91.0% | −0.7%p | 1 |
| **전체** | **863** | **81.2%** | **78.2%** | **−3.0%p** | **45** |

피해가 **여의도중학교에 집중**됩니다. 특히 이번에 복원한 **체육관동 부속(원형부)**
48개 수광점은 43.8% → **0%** 로 전멸합니다. 반대로 여의도여고 A동은 51.5% →
84.8%로 좋아집니다(기존 화랑 5개동이 한 동으로 모이며 그림자가 비켜감).

충족률만 보면 −3.0%p지만 **평균 총일조는 4.32h → 3.91h(−0.41h)** 로, 문턱을
