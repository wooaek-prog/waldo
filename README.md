...
(여의도초는 분석지점도 8×8 = 64점, 나머지 3개교는 8m 격자 근사 732점). 분석지점도를
못 받은 학교의 운동장은 실제 경계가 아니라 근사입니다 — 교사동에서 60m 이내의
...
| 여의도초등학교 | 198 | 93.9% | 93.4% | −0.5%p | 1 | 0 |
| **전체** | **1,659** | **79.3%** | **78.1%** | **−1.2%p** | **69** | **49** |
...
충족률만 보면 −1.2%p지만 **평균 총일조는 4.30h → 4.04h(−0.26h)** 로, 문턱을
넘은 채 시간을 잃는 수광점이 훨씬 많습니다.

**수광점 자료 근거 등급**: 1,659개가 모두 같은 근거가 아닙니다 —
**1,137개(69%)는 도면 없이 추정**한 것입니다(일반식 341 + 운동장 796).
...
| **C+ 운동장 격자 도면** | 칸 수·방위는 분석지점도대로, 경계만 추정 | **64** |
| **C 운동장 근사** | 분석지점도도 없어 옥외지반을 격자로 근사 | **732** |

[docs/school_receptor_provenance.md](docs/school_receptor_provenance.md) 참조.

**운동장 분석지점도 반영**(`school_playground.py` · `data/school_playgrounds.json`):
교육환경평가 옥외체육장 분석지점도는 운동장을 **정해진 칸 수로 나누고 칸마다 한
점**을 찍습니다. 여의도초 도면(8 × 8 = **64점**)을 반영해 종전 8m 격자 근사
228점을 도면과 같은 분모로 맞췄습니다. 다만 도면이 치수·좌표 없는 정투상
3D 뷰라 **경계는 옥외지반 최대 내접 사각형(51 × 51m, 칸 6.4m)으로 추정**한
상태라 등급이 A 가 아닌 C+ 입니다 — 실제 경계를 받으면 `polygon` 에 넣어 A 로
확정합니다.
...
전체로는 **개선**입니다 — 충족 1,316 → 1,329개(+0.8%p), 신규 충족 36 >
신규 불충족 23, 평균 일조 −0.14h(현행 트랙B는 −0.26h).
