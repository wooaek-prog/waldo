...
# 학교별 수광점(受光點) 정의 – 정면도 기반 재정의

일조 분석의 정확도는 그림자 계산보다 **"어디를 교실 창으로 보느냐"** 에 더 크게
좌우됩니다. 학교별 정면도를 받는 대로 종전 일반식(외벽 5m 균등 · 남향계열 외벽
...
| 학교 | 상태 | 수광점 |
| --- | --- | --- |
| 여의도초등학교 (40-3) | 본관·별관 정면도 반영 | 158 → **249** |
| 여의도중학교 (40-1) | 체육관동 4면도 + 본관 정면도 반영 | 249 → **548** |
| 여의도여자고등학교 (40-2) | 도면 대기(일반식) | – |
| 여의도고등학교 (30-1) | 도면 대기(일반식) | – |

여의도중학교 도면에는 치수선이 있어 층별 바닥레벨을 직접 읽었습니다 —
**1층만 3.9m, 2층 이상 3.6m** 로, 종전에 쓰던 3.5m 균일 층고와 다릅니다.
수광 높이도 창 하단~상단의 중앙(본관 바닥 +2.40m)으로 바뀌었습니다.

⚠️ 이 과정에서 AL_D010 에 **용도·층수·높이가 모두 비어 있어 종전 분석에서 통째로
빠져 있던 787.5㎡ 건물**(여의도중 체육관동 부속)을 4면도로 복원했습니다.
수광점 96개를 새로 넣었으나, **그림자를 만드는 건물로는 아직 반영되지 않았습니다** —
일조 재계산 시 함께 넣어야 합니다.
