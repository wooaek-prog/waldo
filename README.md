...
수광점 **1,659개** = 교사동 창면 863(도면기준 522 / 일반식 341) + 운동장 지반 796
(여의도초는 배치도 경계 + 분석지점도 8×8 = 64점, 나머지 3개교는 8m 격자 근사 732점).
분석지점도를 못 받은 학교의 운동장은 실제 경계가 아니라 근사입니다 — 교사동에서
60m 이내의 옥외지반 중 **그 학교가 다른 어떤 건물보다 가까운** 격자점만 남겨 가장
가까운 학교에 배정해, 학교끼리 겹치지 않게 했습니다(`--playground` 로 대체 가능).
...
| **전체** | **1,659** | **78.5%** | **77.5%** | **−1.0%p** | **69** | **52** |
...
충족률만 보면 −1.0%p지만 **평균 총일조는 4.21h → 3.95h(−0.26h)** 로, 문턱을
넘은 채 시간을 잃는 수광점이 훨씬 많습니다.

**수광점 자료 근거 등급**: 1,659개가 모두 같은 근거가 아닙니다 —
**1,073개(65%)는 도면 없이 추정**한 것입니다(일반식 341 + 운동장 근사 732).
...
| A− 도면기준(추론·계측 포함) | 폴리곤 대응·제원을 추론했거나 래스터를 계측 | 145 |
| **B 일반식** | 도면 없음 — 층고·창높이·수평배치 전부 가정 | **341** |
| **C 운동장 근사** | 분석지점도도 없어 옥외지반을 격자로 근사 | **732** |
...
**정해진 칸 수로 나누고 칸마다 한 점**을 찍습니다. 여의도초 도면(8 × 8 =
**64점**)으로 종전 8m 격자 근사 228점을 도면과 같은 분모로 맞췄습니다.
...
종전에는 경계를 몰라 최대 내접 사각형으로 추정했더니 운동장이 교사동 **북동쪽**에
잘못 놓였습니다. 배치도상 실제 위치는 교사동 **남서쪽**이고, 바로잡자 여의도초
운동장 충족률이 **98.4% → 78.1%**(S1)로 내려갔습니다 — 북동쪽 빈 땅은 그림자가
거의 닿지 않던 곳입니다. 나머지 3개교 운동장 경계도 같은 배치도에서 읽어 두었고
(여의도고 6,459㎡ / 여의도중 7,218㎡ / 여의도여고 4,318㎡), 각 학교 분석지점도의
칸 수를 받으면 확정합니다.
...
전체로는 **개선**입니다 — 충족 1,303 → 1,324개(+1.3%p), 신규 충족 44 >
신규 불충족 23, 평균 일조 −0.13h(현행 트랙B는 −0.26h).
